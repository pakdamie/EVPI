...
SAIRV_M_Complex_NVac <-
  function(t, x, params) {
    # This is a function that runs the model for deSolve
    n <- params$n # The number of subcompartments
                  # (related to maximum age - 80)

    #Note that all state compartments have 80 age compartments
    ...
    VacS<- x[((12 * n) + 1):(13 * n)] ### The Vaccine incidence of susceptibles
    VacR<- x[((13 * n) + 1):(14 * n)] ### The Vaccine incidence of recovered
    ...
    #Here is the starting vector that is 0

    v <- rep(0,80)

    #Using the equation -log(1-prop)/DT = r, if we know
    #the proportion that will be vaccinaed and in how many
    #time units we can get the rate (don't get proportion
    #and rate mixed up). So if 50% of the total population
    #is vaccinated in one time step (a year), the rate has to
    #be -log(1-0.5)/1 = 0.69. However, we are vaccinating
    #one age group first.

    #This looks for the first time point where 75% of the
    #first age group will be vaccinated with a total rate
    #of the total population. We  can do this by using the
    #equation DT = -log(1-prop)/r, this tells us depending
    #on the proportion we want vaccinated and the rate,
