...
SAIRV_M_Complex_S <-
  function(t, x, params) {
    ...
    ###Remember each subcompartment is an age class
    ############
    ## States###
    ############
    ...
    -log(100)
      ...
      with(as.list(params), {
      v <- ifelse(sum(Vac) < Vac_All * (sum(Rec[20:80]) * 0.95 +
        sum(Rec[18:80] * 0.02)),
      v, rep(0, 80)
      ...
      if (sum(Vac[Order]) >= 0.50 * sum(S_initial[Order] + R_initial[Order])) {
        Allocation[18:80] <- 1
      }
      else{
      ...
      dS <- c((mu + 1) * (S[n] + SV[n] + SU[n] + I[n] + A[n] + AV[n] + IV[n]
        ...
        Allocation * v * S + omega_V * R + omega_V * SV
      ...
      dSV <- (pv * Allocation * v * S)  +
        ...
        (a + phi_V) * SV - c(rep(0, n - 1), mu) * SV - omega_V * SV + omega_N * RV
      ...
      dSU <- (1 - pv) * Allocation * v * S + (1 - pv) * Allocation * v * R +
        c(0, a[1:(n - 1)] *
      ...
      dR <- gamma * (I + A) + c(0, a[1:(n - 1)] * R[1:(n - 1)]) -
        a * R - c(rep(0, n - 1), mu) * R - omega_N * R - Allocation * v * R
      ### The Vaccinated Recovered Population
      ### We have an inflow depending on gamma (recovery rate) and then
      ### we have the loss due to waning of immunity

       # Vaccinated Recovered Population
      dRV <- (pv * Allocation * v * R) + gamma * (IV + AV) + c(0, a[1:(n - 1)] * RV[1:(n - 1)]) -
        a * RV - c(rep(0, n - 1), mu) * RV - omega_V * RV
      ...
      Vac <- (Allocation * v * S) + (Allocation * v * R) ### The Vaccine incidence
