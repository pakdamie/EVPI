...
SAIRV_M_Complex_NVac_NOR <-
  function(t, x, params) {
    ...
    ###We know vaccine allocation

    with(as.list(params), {

    #The total popoulation
    N = 1
    ...
    v[inc] <-r1
      ...
      dS <- c((mu + 1) * (S[n] + SV[n] + SU[n] + I[n] + A[n] + AV[n] + IV[n]
        + R[n] + RV[n]), rep(0, n - 1)) +
        c(0, a[1:(n - 1)] * S[1:(n - 1)]) -
        (a * S) - (phi_S * S) - (c(rep(0, n - 1), mu) * S) - (v * S) + (omega_N * R) + (omega_V * SV)
      ...
      Vac <- (v * S) + (v * R) ### The Vaccine incidence
