...
SAIRV_M_Complex_NVac <-
  function(t, x, params) {
    ...
    VacS <- x[((11 * n) + 1):(12 * n)] ### The Vaccine incidence
    VacR<- x[((12 * n) + 1):(13 * n)] ### The Vaccine incidence
    ...
    v[inc] <-r1
      ...
      VacS <- (v * S) ### The Vaccine incidence
      VacR <- (v * R)


       res <- c(dS, dSV, dSU, dA, dI, dAV, dIV, dR, dRV, KI, KA, VacS,VacR)
