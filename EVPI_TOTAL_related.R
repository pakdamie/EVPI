...
EVPI_ALL_SUPER <- function(x) {
  ...
  splitted_uncertainty_action <- split(x, x$Var9)
  ...
  EV_C_C <- mean(apply(ACTIONS_C, 1, min))
  EV_C_M <- mean(apply(ACTIONS_M, 1, min))

  ###Average across actions and find the minimium
  ###EV_UC = expected value under UNCERTAINTY
  EV_UC_C <- min(colMeans(ACTIONS_C))
  EV_UC_M <- min(colMeans(ACTIONS_M))
